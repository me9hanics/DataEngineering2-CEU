...
filename = "raw-edits-" + DATE_PARAM + ".txt"
raw_edits_file = RAW_LOCATION_BASE / filename
with open(raw_edits_file, "w") as f:
...
bucket_configuration = {"LocationConstraint": "eu-west-1"}
names = [bucket["Name"] for bucket in s3.list_buckets()["Buckets"]]
if S3_WIKI_BUCKET not in names:
...
s3.upload_file(str(raw_edits_file), S3_WIKI_BUCKET, s3_key)
